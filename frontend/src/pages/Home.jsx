        ...
        <div className="">
          <div className='grid grid-cols-3 gap-10'>
            {exercises.map((exercise) => (
              <div className='border-2 rounded-2xl w-100  gap-10'  
                   key={exercise._id} >
                    
                {/* <h3>{exercise._id}</h3> */}
                <h1 className='text-4xl text-green-600 text-center py-5'>{exercise.template}</h1>
                <h2 className='text-xl text-amber-300 text-center py-5'>{exercise._id}</h2>
                <div className='flex justify-center'>
                ...
                <ol className='text-2xl my-5 mx-5 max-h-60 overflow-y-auto pr-4'>
                  {exercise.exercises.map((name,index) => (
                    <li className='flex justify-between items-center' key={index}>{index + 1}: {name} 
                     <div className='flex gap-1 py-5 text-xl px-4'>
